...
/* putByteArray:
 * Inputes the value of the byte array curByte
 *
 * @params: Byte[] - the byte array of the instruction
 * @params: size - size of the instruction
 * @params: address - the address of the instruction
 * @params: imem_error - bool determining an error in memory
 */
void Memory::putByteArray(uint8_t byte[], int size, int64_t address, bool& imem_error)
{
   if(address + size > MEMSIZE)
   {
       imem_error = true;
   }

    uint8_t curByte = byte[0];
    for(int i = 0; i < size; i++){
        if(address >= 0 && (address + i) < MEMSIZE  && !imem_error)
        {
            mem[address + i] = curByte;
            curByte = byte[i+1];
        }
        else if((address + i) < MEMSIZE)
        {
            imem_error = true;
        }
    }
